cat << EOF > lazarus-workload-image-registry-credentials.yaml
apiVersion: v1
data:
  ca.crt: 
  password: dGVzdAo=
  username: dGVzdAo=
kind: Secret
metadata:
  labels:
    cluster.x-k8s.io/cluster-name: lazarus-workload
    cluster.x-k8s.io/provider: nutanix
  name: lazarus-workload-image-registry-credentials
  namespace: lazarus-workspace
type: Opaque
EOF

kubeseal --format yaml --scope cluster-wide < lazarus-workload-image-registry-credentials.yaml > lazarus-workload-image-registry-credentials-sealed.yaml